...
export function InvestmentCompareCalculator() {
  ...
  return <div className="comparison-lab"><section className="input-panel"><div className="panel-heading"><div><span>STEP 01 · OPTIONS</span><h2>최대 세 가지 투자안을 같은 기준으로 입력해 주세요</h2></div></div>
    ...
    <QuickAssumptionNote mode={inputMode}>수익 과세율 15.4%를 사용하고 세후 예상자산·손실액만 비교합니다. 유동성·시간·레버리지·판단근거 비교지수는 상세 계산에서만 사용합니다.</QuickAssumptionNote>
  </section><section className="result-panel" aria-live="polite"><div className="panel-heading"><div><span>STEP 02 · COMPARISON</span><h2>입력 순서대로 보는 시나리오 비교</h2></div></div><QuickEstimateNotice mode={inputMode} /><div className="verdict-line">{inputMode === "quick" ? <><span>빠른 비교 기준</span><strong className="verdict">예상자산·손실</strong><p>빠른 계산에서는 숨겨진 정성 항목으로 순위를 만들지 않습니다. 비교지수는 상세 계산에서만 표시합니다.</p></> : <><span>사용자 가중치</span><strong className="verdict">합계 {weightTotal.toFixed(1)}</strong><p>비교지수는 순위나 투자 추천이 아니라 사용자가 정한 중요도를 같은 척도에 적용한 참고값입니다.</p></>}</div>
    <div className="candidate-results">{results.map((item, index) => <article key={`${item.name}-${index}`} className="candidate-result"><div><span>투자안 {index + 1}</span><h3>{item.name}</h3></div><strong>{inputMode === "quick" ? "예상자산·손실 중심 비교" : item.comparisonIndex === null ? "가중치 미설정" : `비교지수 ${item.comparisonIndex.toFixed(1)}`}</strong><dl><div><dt>{years}년 세후 예상자산</dt><dd>{formatMoney(item.afterTax)}</dd></div><div><dt>추정 과세액</dt><dd>{formatMoney(item.tax)}</dd></div><div><dt>기간 중 추정 최대노출 기준 손실액</dt><dd>−{formatMoney(item.lossAmount)}</dd></div><div><dt>손실 계산 기준 자산</dt><dd>{formatMoney(item.exposureBase)}</dd></div><div><dt>현금흐름 원금회수기간</dt><dd>{item.paybackMonths ? `${Math.ceil(item.paybackMonths / 12)}년` : "현금흐름만으로 계산 불가"}</dd></div>{inputMode === "detailed" && <div><dt>근거·철수기준 지수</dt><dd>{item.factors.decisionQuality.toFixed(0)} / 100</dd></div>}</dl></article>)}</div>
